...
async def on_lang(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
...
async def on_donate(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
...
async def on_change_lang(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
...
async def on_new_check(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    lang = lang_of(context)
    context.user_data["topic"] = None
...
async def receive_essay(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    lang = lang_of(context)
...
async def fallback_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if not context.user_data.get("lang"):
        ...
        return ConversationHandler.END
    return await receive_topic(update, context)
...
def main() -> None:
    ...
    incoming = filters.TEXT | filters.PHOTO | filters.Document.IMAGE
    conv = ConversationHandler(
        entry_points=[
            CommandHandler("start", cmd_start),
            CallbackQueryHandler(on_lang, pattern=r"^lang_(az|ru|en)$"),
            CallbackQueryHandler(on_donate, pattern=r"^donate$"),
            CallbackQueryHandler(on_change_lang, pattern=r"^change_lang$"),
            CallbackQueryHandler(on_new_check, pattern=r"^new_check$"),
        ],
        states={
            WAIT_TOPIC: [
                MessageHandler(incoming & ~filters.COMMAND, receive_topic),
                CallbackQueryHandler(on_donate, pattern=r"^donate$"),
                CallbackQueryHandler(on_new_check, pattern=r"^new_check$"),
                CallbackQueryHandler(on_change_lang, pattern=r"^change_lang$"),
                CallbackQueryHandler(on_lang, pattern=r"^lang_(az|ru|en)$"),
            ...
            WAIT_ESSAY: [
                MessageHandler(incoming & ~filters.COMMAND, receive_essay),
                CallbackQueryHandler(on_donate, pattern=r"^donate$"),
                CallbackQueryHandler(on_new_check, pattern=r"^new_check$"),
                CallbackQueryHandler(on_change_lang, pattern=r"^change_lang$"),
                CallbackQueryHandler(on_lang, pattern=r"^lang_(az|ru|en)$"),
        ...
        fallbacks=[
            CommandHandler("start", cmd_start),
            MessageHandler(incoming & ~filters.COMMAND, fallback_message),
        ],
        allow_reentry=True,
    )
