...
# One Gemini model handles BOTH text grading and photo reading (no more
# juggling a separate vision model with its own separate rate limit).
# "gemini-flash-latest" is Google's rolling alias for the current stable
# Flash model, kept as a second candidate in case the pinned version name
# below is ever retired.
MODELS = _model_list("GEMINI_MODEL", "gemini-2.5-flash,gemini-flash-latest")
...
def _generate_with_fallback(models: list[str], make_kwargs):
    """Try each model in order; only raise if every single one fails."""
    client = gemini_client()
    last_error: Exception | None = None
    for model in models:
        try:
            response = client.models.generate_content(model=model, **make_kwargs())
            if model != models[0]:
                log.warning("Fell back to model %s (first choice unavailable)", model)
            return response
        except Exception as exc:
            # Only swallow genuine "try the next model" cases: not-found /
            # permission / rate-limit style client errors. Anything else
            # (bad request shape, auth entirely missing, etc.) should
            # surface immediately instead of silently trying every model.
            status = getattr(exc, "code", None) or getattr(exc, "status_code", None)
            if isinstance(exc, GeminiClientError) or status in (404, 429, 403):
                log.warning("Model %s unavailable right now (%s), trying next", model, exc)
                last_error = exc
                continue
            raise
...
async def evaluate_essay(topic: str, essay: str, lang: str) -> dict:
    ...
    try:
        ...
        log.warning("First attempt returned no JSON, retrying once")
        raw = await asyncio.to_thread(_call)
        data = parse_json(raw)
